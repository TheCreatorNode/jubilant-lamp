...
async function createProfile(req, res) {
  ...
  const existing = db.find((p) => p.name === lower);
  if (existing) {
    return res.status(200).json({
      status: "success",
      message: "Profile already exists",
...
async function getProfile(req, res) {
  ...
  const profile = db.find((p) => p.id === id);
...
async function listProfiles(req, res) {
  ...
  const filtered = db.filter((p) => {
    if (gender && p.gender?.toLowerCase() !== gender.toLowerCase())
      return false;
    if (country_id && p.country_id?.toLowerCase() !== country_id.toLowerCase())
      return false;
    if (age_group && p.age_group?.toLowerCase() !== age_group.toLowerCase())
      return false;
    return true;
  });
